...
export const sendMessage = async (req, res) => {
  try {
    ...
    if (image) {
      ...
      const uploaadResponse = await cloudinary.uploader.upload(image);
      imageUrl = uploaadResponse.secure_url;
    }
    ...
    if (recieverSocketId) {  // if user is online, send real-time message
  ...
  try {
    ...
    const chatPartnerIds = [
      ...new Set(
        messages.map(
          (
            msg, // split and put them in a set to get unique values
          ) =>
